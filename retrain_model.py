...
# Where your flat 1K images + labels currently live (before splitting)
RAW_IMAGES_DIR = "dataset/images/train"
RAW_LABELS_DIR = "dataset/labels/train"
...
EPOCHS       = 100      
IMAGE_SIZE   = 768      # Optimized for structural precision
BATCH_SIZE   = 16       # Increased batch size due to lower resolution
VAL_SPLIT    = 0.2      
PATIENCE     = 50       
LR0          = 0.0005   
WORKERS      = 4        
...
def run_training():
    ...
    results = model.train(
        ...
        #  Geometry (VERY IMPORTANT for infrastructure)
        degrees     = 8,        # small rotation only to keep ground-truth verticality
        translate   = 0.1,
        scale       = 0.1,

        #  Flips
        fliplr      = 0.5,
        flipud      = 0.0,       # NEVER flip vertically (poles must point up)

        #  Structure-safe
        mosaic      = 0.3,       # Reduced mosaic to maintain object structure
        mixup       = 0.0,       # Disabled mixup to prevent blending noise
