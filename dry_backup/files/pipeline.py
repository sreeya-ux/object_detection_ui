...
class InfrastructurePipeline:
    ...
    def predict(
        ...
        # Dedicated detector for small hardware. 
        # Increase imgsz to 1280 to catch thin wires/conductors.
        raw_insulator = self.insulator_detector(image_path, conf=0.10, imgsz=1280, verbose=False)
        ...
        for result in list(raw640) + list(raw1280) + list(raw1600):
            ...
            if obb is not None and len(obb) > 0:
                for i in range(len(obb)):
                    ...
                    # Conductor Sensitivity Boost: 
                    # Accept conductors @ 0.02+, others @ 0.10+
        ...
        for idx, (b, c, a) in enumerate(insulator_boxes):
            print(f"  Raw[{idx}] Box={b}, Conf={c:.3f}")

        # ── Deduplicate multi-scale detections (NMS) ─────────
        # We use a standard threshold (0.50) to merge overlapping detections from different models
        insulator_boxes = self._nms(insulator_boxes, iou_threshold=0.50)
        ...
        conductor_boxes = self._nms(conductor_boxes, iou_threshold=0.45)

        # ── Infer pole if missing (for partial/cut-off photos) ─────
        if not pole_boxes_raw and (insulator_boxes or crossarm_boxes):
            inferred_pole = self._infer_pole_if_missing(
                insulator_boxes, crossarm_boxes, img_h, img_w
            )
            if inferred_pole:
                pole_boxes_raw.append(inferred_pole)
                flags["inferred_pole"] = True

        # ── Classify each insulator ───────────────────────────
        insulator_results = []
        for box, conf_val, angle_deg in insulator_boxes:
            # Use img_original for shed counting (don't break the model)
        ...
        crossarm_results = []
        for box, _, ang in crossarm_boxes:
    ...
    def _categorise(
        ...
        if _match_keyword(cls_name, "insulator"):
            ...
            crossarm_boxes.append((box, conf_val, angle_deg))
    ...
    def _draw(
        ...
        if result.pole_orientation:
            ...
            # Use dashed line effect for inferred pole
            if is_inferred:
            ...
            cv2.putText(vis, label,
                        (x1, y2 + 16), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
