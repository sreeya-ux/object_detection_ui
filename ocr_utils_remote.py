...
class PoleOCR:
    ...
    def _request_azure_text(self, crop_bgr):
        """Send a tightly-cropped image to Azure Mistral OCR and return raw text."""
        from config import USE_AZURE_MISTRAL_OCR, AZURE_MISTRAL_API_KEY, AZURE_MISTRAL_ENDPOINT, AZURE_MISTRAL_MODEL
        if not USE_AZURE_MISTRAL_OCR or not AZURE_MISTRAL_API_KEY or crop_bgr is None or crop_bgr.size == 0:
            return ""

        try:
            crop_bgr = self._resize_for_ocr(crop_bgr)
            h, w = crop_bgr.shape[:2]
            ...
            print(f"[OCR] Sending crop to Mistral OCR ({w}x{h})...")
            r = requests.post(AZURE_MISTRAL_ENDPOINT, headers=headers, json=payload, timeout=10)
            if r.status_code != 200:
                print(f"[OCR] Azure Mistral failed: {r.status_code} - {r.text}")
                return ""
            ...
            markdown_text = pages[0].get("markdown", "")
            # Clean markdown formatting like '# ROSS US\n\n![img-0.jpeg](img-0.jpeg)'
            clean_text = re.sub(r'!\[.*?\]\(.*?\)', '', markdown_text)
            clean_text = clean_text.replace("#", "").strip()
            return clean_text

        except Exception as e:
            print(f"[OCR] Azure Mistral OCR error: {e}")
            return ""

    def _read_with_azure_mistral(self, crop_bgr):
        """Use Azure Mistral Document AI OCR API to read the painted pole ID."""
        clean_text = self._request_azure_text(crop_bgr)
        if not clean_text:
            return "Not Found"
        normalized = self._normalize_pole_id(clean_text)
        if normalized != "Not Found":
            print(f"[OCR] Azure Mistral ID: '{normalized}' from '{clean_text}'")
        return normalized
    ...
    def process_pole_tag(self, image, box):
        """
        Read the hand-painted ID on a detected pole.
        ...
        try:
            ...
            for crop in unique_lines[:4]:
                local_text = self._read_with_easyocr(crop)
                if local_text:
                ...
                raw_text = self._request_azure_text(crop)
                if not raw_text:
                    continue
                if not top_text and self._looks_like_rdss(raw_text):
                    top_text = raw_text
                if not bottom_text and self._extract_digits(raw_text):
                    bottom_text = raw_text
                normalized = self._normalize_pole_id(raw_text)
                if normalized != "Not Found":
            ...
            for crop in unique_candidates:
                result = self._read_with_azure_mistral(crop)
                if result != "Not Found":
                    score = self._pole_score(result)
                    if score > best_score:
                        best_score = score
                        best_result = result

            if best_result != "Not Found":
                print(f"[OCR] Azure Mistral selected best ID: '{best_result}' (score={best_score})")
                return best_result
