...
class InfrastructurePipeline:
    ...
    def predict(self, image_path, visualize=True, save_path=None, fast_mode=False) -> PipelineResult:
        ...
        main_pole = all_poles_res[0] if all_poles_res else None

        # Run local OCR on detected poles to read the painted ID.
        # We run OCR on all detected poles and choose the candidate with the highest non-negative OCR score.
        pole_id_str = "Not Found"
        best_ocr_score = -9999
        h_full, w_full = img_original.shape[:2]

        for p in all_poles_res:
            cand_id = self.ocr.process_pole_tag(img_original, p.box)
            if cand_id != "Not Found":
                score = self.ocr._pole_score(cand_id)
                print(f"[Pipeline OCR] Pole at {p.box} read tag: '{cand_id}' (score={score})")
        ...
        if not all_poles_res and (pole_id_str == "Not Found" or not any(ch.isdigit() for ch in pole_id_str)):
            ...
            if full_img_id != "Not Found":
                ...
                print(f"[Pipeline OCR] Full image fallback read tag: '{full_img_id}' (score={score})")
                if score > best_ocr_score:
                    best_ocr_score = score
                    pole_id_str = full_img_id
